...
internal static class ConfigurationManager
{
	...
	public static void Load(string filename, Feature[] features, bool warnIfNotExists = true)
	{
		try
		{
			...
			foreach (var feature in features)
			{
				...
				foreach (var op in properties)
				{
					...
					{
						Context.AddConsoleLog(string.Format(Strings.ErrorCorruptedPropertyFormat, key, filename).Red());
					}
				}
			}
		...
		{
			Context.AddConsoleLog(string.Format(Strings.ErrorCannotLoadFormat, filename, ioe.Message).Red());
		}
	}
	...
	public static void Save(string filename, Feature[] features)
	{
		...
		{
			Context.AddConsoleLog(string.Format(Strings.ErrorCannotSaveFormat, filename, ioe.Message).Red());
		}
	}
